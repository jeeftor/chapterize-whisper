...
def validate_directory(ctx, param, value: Optional[str]) -> str:
    ...
    if not directory:
        raise click.BadParameter(
            "Directory must be provided via --dir or BOOK_DIRECTORY environment variable"
        )
...
@click.option(
    "--device", default="auto", help="Device to use (cpu, cuda, auto)", show_envvar=True
)
@click.option(
    "--num-workers",
    default=8,
...
async def async_detect(
    dir: str, model: str, device: str, num_workers: int, cpu_threads: int
):
...
async def async_upload(dir: str, api_key: str, abs_url: str, book_id: str | None):
    ...
    console.print(
        "[green]Running upload mode with the following configuration:[/green]"
    )
    console.print(f"Directory: {dir}")
    console.print(
        f"API Key: {api_key[:4]}{'*' * (len(api_key) - 4)}"
    )  # Mask the API key
